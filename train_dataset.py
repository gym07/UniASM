...
if __name__ == '__main__':
    ...
    train_size = round(len(all_ids) * 0.9) # 90% for train
    if train_size % 2 != 0:
        train_size += 1
    valid_size = len(all_ids) - train_size
    if valid_size % 2 != 0:
        valid_size -= 1
    create_datasets("./testdata/train-{}.tfrec".format(train_size), all_ids[:train_size], all_seg[:train_size])
    create_datasets("./testdata/validate-{}.tfrec".format(valid_size), all_ids[train_size:train_size + valid_size], all_seg[train_size:train_size + valid_size])
